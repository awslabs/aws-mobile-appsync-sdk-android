...
public final class RealAppSyncCall<T> implements AppSyncQueryCall<T>, AppSyncMutationCall<T> {
  ...
  @Override public void enqueue(@Nullable final GraphQLCall.Callback<T> responseCallback) {
    try {
  ...
  private ApolloInterceptor.CallBack interceptorCallbackProxy() {
    return new ApolloInterceptor.CallBack() {
      ...
      @Override public void onCompleted() {
        ...
        callback.get().onStatusEvent(GraphQLCall.StatusEvent.COMPLETED);
      }

      @Override public void onFetch(final ApolloInterceptor.FetchSourceType sourceType) {
        responseCallback().apply(new Action<GraphQLCall.Callback<T>>() {
          @Override public void apply(@Nonnull GraphQLCall.Callback<T> callback) {
            switch (sourceType) {
              case CACHE:
